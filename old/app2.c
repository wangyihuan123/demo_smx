...
#include "mstp_lc_ddl.h" /* for STP_OUT_flush_lt */
...
flush (STATE_MACH_T *this, char* reason) /* 17.19.9 */
{
  ...
  bret = STP_OUT_flush_lt (port->port_index, port->owner->mstid,
...
setTcPropBridge (STATE_MACH_T* this, char* reason) /* 17.19.14 */
{
  ...
  STP_OUT_flush_lt (port->port_index, port->owner->mstid,
...
newTcWhile (STATE_MACH_T* this) /* 17.19.7 */
{
  ...
  if ((port->sendMSTP || port->sendRSTP) && port->operPointToPointMac) {
