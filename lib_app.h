...
/* STP machine instance : bridge per MSTID: 17.17 */
...
#include "mstp_lib_port.h"
#include "mstp_lib_rolesel.h"
...
typedef struct stpm_t {
  ...
  Bool 					begin; /* 13.23.1 */
#define MST_CONFIG_ID_LEN	51  
  char 					MstConfigId[MST_CONFIG_ID_LEN+1]; /* 13.23.8 */
  BRIDGE_ID             BrId;           /* 17.17.2 */
  TIMEVALUES_T          BrTimes;        /* 17.17.4 */
  PORT_ID               rootPortId;     /* 17.17.5 */
#ifdef SLR_STP_CODE
  PORT_ID               masterPortId;     /* 17.17.5 */
#endif
...
STPM_T*
STP_stpm_create (int mstid, char* name);
